...
jeżeli hosty są w tej samej sieci, to nie ma problemu, jak nie to możliwe, że trzeba dodatkowych mechanizmów, np. Proxy ARP
...
_wydaje mi się że jeśli są w różnych sieciach to jest potrzebny proxy ARP ? => TODO sprawdz_
_bo wtedy router klamie z MAC_
...
- proxy arp może być ustawione na routerze

### Różnica z normalnym ARP

- w normalnym ARP, hosty wiedzą, że są w innej sieci i wysyłają zapytanie ARP do bramy domyślnej (debil: routera)
- tutaj zapytanie nie jest kierowane do bramy tylko do proxy, które udaje, że jest celem w tej samej sieci
